...
public class AccountService {
    ...
    public ResponseEntity<Map<String, Object>> updateAccount(Account account) {
        try {
    ...
    @Transactional
    public ResponseEntity<Map<String, Object>> deleteAccount(int id) {
        try {
    ...
    public ResponseEntity<Map<String, Object>> deleteContact(int id) {
        try {
    ...
    public ResponseEntity<Map<String,Object>> getAccount(int accountId){
    	try {
    		String sql="SELECT a.id , a.firstname , a.lastname, a.username, c.type as contact_type, c.contact as contact_contact " + 
    				   "FROM Account a LEFT JOIN Contact c ON a.id = c.account_id " + 
    				   "WHERE a.id = ? AND c.primary_contact = true";
    		
    		List<Map<String,Object>> accounts = jdbcTemplate.queryForList(sql,accountId);
    		Map<String,Object> response = new HashMap<>();
    		response.put("success", true);
    		response.put("items", accounts);
    		return ResponseEntity.ok(response);
    		...
    		Map<String,Object> response = new HashMap<>();
    		response.put("success", false);
    		response.put("message", "Error fetching account with id " + accountId + e.getMessage());
    		return ResponseEntity.badRequest().body(response);
    	}
    }
